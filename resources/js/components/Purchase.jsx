...
class Purchase extends Component {
    ...
    handleClickSubmit(printReceipt = false) {
        ...
        Swal.fire({
            ...
            confirmButtonText: printReceipt
                ? translations["confirm_pay_print"] || translations["confirm_pay"]
            ...
            if (result.value) {
                const { receipt_url, message } = result.value;
                const successTitle = message || translations["sale_completed"] || "Success";
                const printLabel = translations["print_receipt"] || "Print receipt";
                const viewLabel = translations["view_receipt"] || printLabel;

                if (printReceipt && receipt_url) {
                    const printWindow = window.open(receipt_url, "_blank", "noopener,noreferrer");
                    if (printWindow) {
                        printWindow.focus();
                        Swal.fire({
                            icon: "success",
                        ...
                        Swal.fire({
                            icon: "info",
                            title: successTitle,
                            html: `<a href="${receipt_url}" target="_blank" rel="noopener noreferrer">${printLabel}</a>`,
                            showConfirmButton: true,
                            confirmButtonText: printLabel,
                            showCancelButton: true,
                    ...
                    if (receipt_url) {
                        Swal.fire({
                            icon: "success",
                            title: successTitle,
                            html: `<a href="${receipt_url}" target="_blank" rel="noopener noreferrer">${viewLabel}</a>`,
                            showConfirmButton: true,
                            confirmButtonText: viewLabel,
                            showCancelButton: true,
                            cancelButtonText: translations["cancel"] || "Cancel",
                        }).then((dialogResult) => {
                            if (dialogResult.isConfirmed) {
                    ...
                    <div className="row">
                        ...
                        <div className="col-12 col-md-4 mb-2">
                            <button
                                ...
                                disabled={!cart.length}
                                onClick={() => this.handleClickSubmit(false)}
                            >
                                {translations["receive_payment"] || "Receive Payment"}
                            </button>
                        </div>
                        <div className="col-12 col-md-4 mb-2">
                            <button
                                ...
                                disabled={!cart.length}
                                onClick={() => this.handleClickSubmit(true)}
                            >
                                {translations["receive_and_print"] || "Receive & Print"}
                            </button>
                        </div>
                    </div>
                </div>
